...
client.on('ready', async () => {
  ...
  const activityType = activityTypes[config.activity.type] || ActivityType.Playing;
...
// Helper utility to safely clear and delete old messages
  ...
  client.on('messageCreate', async (message) => {
    ...
    if (command === 'play') {
      ...
      try {
        ...
        if (!player) {
          player = riffy.createConnection({ guildId: message.guild.id, voiceChannel: message.member.voice.channel.id, textChannel: message.channel.id, deaf: true });
        }
